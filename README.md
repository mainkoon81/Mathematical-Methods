...
__a> Stationarity__
 - In a stationary time series, there is no systematic change in **mean** (no trend), no systematic change in **variance**, and no **fluctuation**(periodic variations)...All random variables(![formula](https://render.githubusercontent.com/render/math?math=\X_1,\X_2,..)) have their own unique distribution? So..basically, picking up one of the curves, **`the properties of one section of a data are much like the properties of the other sections of the data`**, and the association b/w such consecutive data points should be none(coz each point follows different distribution?). Usually "stationarity" is a property of a stochastic process of a model, not a time series. But we say `stationary time series` if you think that it can be modeled with **`stochastic process`**...  
